/****Function Check Elements***/
	...
	function checkBounds(entity, field){
		if(isBounds(entity, field)){
			if ( (entity.name.startsWith('player')) || (entity.name.startsWith('enemy')) ){
				entity.angle=calculateAng(entity, field);
				entity.updatePath();
				return false;
			}
			else if(entity.name.startsWith('asteroid')){
				if( (entity.x>field.right) || (entity.y>field.bottom) ){
					return true;
				}
				else {
					return false;
